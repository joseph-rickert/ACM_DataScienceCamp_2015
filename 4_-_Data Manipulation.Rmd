...
Add a new variable, Volatility, to a data frame. "$" is used to index into the data frame.   
...
We want a data frame that only includes prices after January 1, 2000. The first thing we do is check to see what data types the varables are. Noticing that Date is a factor, we willmake it a date as we build a new data frame.  
...
```
What did aggregate do? Let's do one calculation by hand.
```{r}
...
syntax. Many of these packages are organized under the concept of "Tidy Data"   
http://vita.had.co.nz/papers/tidy-data.pdf Here we show another way to move between wide and long format using the functions in Hadley's reshape2 package. For a comprehensive system for working with data frames see the Hadley's dplyr package.   http://cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html
```{r}
library(reshape2)
